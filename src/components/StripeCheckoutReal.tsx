...
export default function StripeCheckout({ isOpen, onClose, selectedPlan }: StripeCheckoutProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!stripe || !elements) {
      alert('Stripe no está disponible. Por favor, recarga la página.');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/create-payment-intent', {
        ...
        body: JSON.stringify({
          ...
          entityData: {
            nombreEntidad: formData.nombreEntidad,
          ...
          kitDetails: formData.kitComunicacion ? {
            colorPrimario: formData.colorPrimario,
            logoPersonalizado: formData.logoPersonalizado,
            materialesPersonalizados: formData.materialesPersonalizados
          } : null
        }),
      });
      ...
      if (paymentIntent.status === 'succeeded') {
        // 3. Confirmar el pago exitoso en el servidor
        ...
        alert(`¡Pago procesado exitosamente! 🎉\n\n✅ Plan: ${currentPlan.title}\n💰 Total: ${total.toFixed(2)}€ (primer pago)\n📦 Kit de comunicación: ${formData.kitComunicacion ? 'Sí (+49€)' : 'No'}\n📅 Segundo pago: ${total.toFixed(2)}€ en 6 meses\n\n🚀 Tu implementación LOPIVI comenzará en las próximas 24 horas.\n📧 Recibirás un email de confirmación con todos los detalles.\n\nID de transacción: ${paymentIntent.id}`);
      ...
      alert(`Error al procesar el pago: ${errorMessage}\n\nPor favor, verifica tus datos e intenta de nuevo, o contacta con soporte técnico.`);
    } finally {
  ...
  const renderStep4 = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">Proceso seguro con Stripe</p>
      </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Powered by Stripe. Tu información está segura y encriptada.
          </p>
        </div>
      </div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
        <div className="flex items-center">
          ...
          <span className="text-green-800 font-medium text-sm">
            Pago 100% seguro procesado por Stripe con encriptación SSL
          </span>
        </div>
      </div>
    </div>
  ...
  // Mantener las otras funciones renderStep1, renderStep2, renderStep3 del componente original
  const renderStep1 = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <DialogHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <DialogDescription>
                Implementación LOPIVI completa en 24 horas - Pago procesado por Stripe
              </DialogDescription>
            </div>
